...
| **Phase 2** | E1–E4 policy/cost plane; ST-1–ST-3 KPI measurement on a realistic trace | **Done at the software/simulation level.** E1 (Policy DSL) and E2 (EPE) were already complete from Phase 0. E3 (KPI meter) is now wired: `aegis/kpi.py`'s `KPIMeter.record()` was previously defined but never called anywhere in the codebase — `policy/engine.py::_route()` now calls it automatically after every successful recovery, using each layer's real *measured* `recovery_secs` (§4.5.4 of test_suite.md: "measured, not modeled"), scaled by a new `OperatorPolicy.gpu_count` field. On this dev machine those measurements are real CPU/MPS-proxy timings, not hardware recovery durations — documented in `kpi.py`'s docstring so the numbers aren't over-claimed. E4 (dashboard) is new: `aegis/dashboard.py` + `aegis.dashboard(fmt="text"\|"json")`, an in-process report (no daemon/HTTP surface, consistent with `ui.md` §6's explicit rejection of a sidecar). ST-3: the `bench/` framework (`SimulationEngine`, FT-POISSON/BURST/PRODUCTION traces, cost model, system adapters for AEGIS/TorchFT/R²CCL/MeCeFO/TierCheck/Vanilla, report generator) already existed from earlier out-of-order work — `bench/run_st3.py` is new and actually runs it end-to-end on FT-PRODUCTION across W1/W2/W3, producing `bench/reports/ft_production_report.md`. Headline: AEGIS beats B-VANILLA on all three workloads (e.g. W2/32-GPU: $116.40 saved, 92.5% faster recovery, over 14 faults) and beats every single-primitive baseline too. **Caveat, stated in the report itself:** only the AEGIS adapter drives the real `AegisRuntime`/EPE; all six systems' recovery-time inputs are `bench/sim/cost_model.py`'s *target* numbers, not hardware measurements — this is the same pre-hardware-validation limitation Phase 1 has, not a new one. `test_suite.md` names the target real-hardware cluster (SeaWulf's A100 partition) for validating these numbers for real — no cluster driver (Slurm scripts, etc.) exists yet, so that half of `test_suite.md` is still design, not code. `eval_design.md` extends this further into a real-inference (GPT-OSS/GLM) evaluation, still pending answers to its open items. |
| **Phase 3** | Learned classifier, predictive pre-staging, straggler tier, inference path | **Not started.** |

**Net effect:** the project did not follow its own build order, but Phase 1 and
Phase 2 have both now landed at the software/simulation level. The `bench/`
$/GPU-hr numbers are still simulated cost-table numbers for all six systems in
`run_st3.py` (the AEGIS adapter's *routing decision* is real, its *timing
input* is not), and the live `KPIMeter` wired into the actual runtime measures
real-but-proxy (CPU/MPS, not GPU) recovery times. Both are honestly labeled
as such rather than presented as validated numbers. The actual remaining
blocker to a credible, sellable $/GPU-hr number is real A100/IB cluster
validation (`eval_design.md`/`test_suite.md` §4.5) — that's the only thing
left that isn't more software composition.
